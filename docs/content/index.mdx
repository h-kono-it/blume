...
Every Blume site can emit [`llms.txt` and `llms-full.txt`](/guides/ai) so coding
...
files, [search](/guides/search) works in dev and production without a hosted
...
- **Components** — callouts, cards, steps, tabs, accordions, badges, file trees,
  and API-reference blocks, usable in MDX with [no imports](/reference/components).
